...
import {
  ...
  ScanLine, Fingerprint, ArrowDownLeft, Key,
} from 'lucide-react'
...
function getCardExpiry(acc: CurrencyAccount) {
  try {
    const d = new Date((acc as { created_at?: string }).created_at ?? Date.now())
    d.setFullYear(d.getFullYear() + 3)
    return `${String(d.getMonth() + 1).padStart(2, '0')}/${String(d.getFullYear()).slice(-2)}`
  } catch { return '12/28' }
}

function getUserInitials(user: { user_metadata?: { full_name?: string }; email?: string } | null) {
...
function getUserDisplayName(user: { user_metadata?: { full_name?: string }; email?: string } | null) {
      ...
      <div className="relative h-full flex flex-col justify-between p-5">
        ...
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <p className="font-semibold text-white" style={{ fontSize: 11, lineHeight: 1.3 }}>{name}</p>
              <p style={{ color: 'rgba(255,255,255,0.55)', fontSize: 9 }}>{curr?.name}</p>
            </div>
          </div>
          <div className="flex flex-col items-end gap-1.5">
            <span className="font-bold px-2.5 py-0.5 rounded-full" style={{ background: 'rgba(255,255,255,0.2)', color: 'rgba(255,255,255,0.9)', fontSize: 10 }}>
              {acc.currency}
            </span>
            {/* EMV chip */}
            <svg width="28" height="20" viewBox="0 0 28 20" fill="none">
              <rect x=".5" y=".5" width="27" height="19" rx="3.5" fill="rgba(255,255,255,0.18)" stroke="rgba(255,255,255,0.3)"/>
              <line x1=".5" y1="7"  x2="27.5" y2="7"  stroke="rgba(255,255,255,0.3)"/>
              <line x1=".5" y1="13" x2="27.5" y2="13" stroke="rgba(255,255,255,0.3)"/>
              <line x1="9"  y1=".5" x2="9"    y2="19.5" stroke="rgba(255,255,255,0.3)"/>
              <line x1="19" y1=".5" x2="19"   y2="19.5" stroke="rgba(255,255,255,0.3)"/>
            </svg>
          </div>
        </div>
        {/* Balance */}
        ...
        <div className="flex items-end justify-between">
          ...
          <p style={{ color: 'rgba(255,255,255,0.35)', fontSize: 9, letterSpacing: '0.05em' }}>{exp}</p>
        </div>
      </div>
    </div>
  )
...
function CardBack({ acc, height = 210 }: { acc: CurrencyAccount; height?: number }) {
  ...
  return (
    <div className="relative rounded-[1.75rem] overflow-hidden w-full select-none"
      ...
      {/* Magnetic stripe */}
      <div className="absolute left-0 right-0" style={{ top: 44, height: 44, background: 'rgba(0,0,0,0.85)' }} />
      {/* Signature + CVV */}
      <div className="absolute left-5 right-5" style={{ top: 102 }}>
      ...
      <div className="absolute left-5 right-5 bottom-4 flex justify-between items-end">
        ...
        <p className="font-bold text-white" style={{ fontSize: 11, letterSpacing: '0.08em', opacity: 0.8 }}>FamillyBill HT</p>
      </div>
    </div>
  )
}
...
// ── Sub-screen overlay wrapper ─────────────────────────────────────────────────
function ScreenOverlay({ children, onBack, title }: { children: React.ReactNode; onBack: () => void; title: string }) {
  return (
    <div className="fixed inset-0 z-40 flex flex-col animate-slide-right" style={{ background: '#F3F3F6' }}>
      {/* Header */}
      <div className="flex items-center gap-3 px-4 pt-12 pb-4" style={{ background: '#fff', borderBottom: '1px solid #F3F3F6' }}>
        ...
        <h1 className="font-bold text-base flex-1" style={{ color: '#0D1B4B', letterSpacing: '-0.02em' }}>{title}</h1>
      </div>
...
// ── Details Screen ─────────────────────────────────────────────────────────────
function DetailsScreen({ acc, user, onBack }: { acc: CurrencyAccount; user: { user_metadata?: { full_name?: string }; email?: string } | null; onBack: () => void }) {
  ...
  const fields = [
    { label: 'Titulaire', value: name, mask: false, show: true, onToggle: () => {} },
    { label: 'Numéro de portefeuille', value: num, mask: !showNum, show: showNum, onToggle: () => setShowNum(v => !v) },
    { label: 'Date d\'expiration', value: exp, mask: false, show: true, onToggle: () => {} },
    { label: 'CVV', value: cvv, mask: !showCVV, show: showCVV, onToggle: () => setShowCVV(v => !v) },
    { label: 'Devise', value: `${curr?.flag ?? ''} ${acc.currency} — ${curr?.name ?? ''}`, mask: false, show: true, onToggle: () => {} },
  ]
    ...
    <ScreenOverlay title="Détails du portefeuille" onBack={onBack}>
      <div className="px-4 pt-5 pb-8 space-y-5">
        ...
        <div className="rounded-2xl overflow-hidden" style={{ background: '#fff' }}>
          {fields.map(({ label, value, mask, onToggle }, i) => (
            <div key={label}
              className="flex items-center px-4 py-4"
              style={{ borderBottom: i < fields.length - 1 ? '1px solid #F3F3F6' : 'none' }}>
              <div className="flex-1 min-w-0">
                <p className="text-xs font-medium mb-0.5" style={{ color: 'rgba(13,27,75,0.45)' }}>{label}</p>
                <p className="font-semibold text-sm font-mono truncate" style={{ color: '#0D1B4B', letterSpacing: mask ? '0.12em' : '0' }}>
                  {mask ? '•••• •••• ••••' : value}
                </p>
              </div>
              {(label.includes('Numéro') || label === 'CVV') && (
...
function SettingsScreen({
  acc, user, onBack, onBlock, onCardBack, onFullSettings,
}: {
  acc: CurrencyAccount
  user: { user_metadata?: { full_name?: string }; email?: string } | null
  onBack: () => void
  ...
  function toggleFrozen() {
    const next = !frozen
    setFrozen(next)
    localStorage.setItem(`fb-frozen-${acc.id}`, String(next))
  }
  function toggleOnline() {
    const next = !online; setOnline(next)
    localStorage.setItem(`fb-online-${acc.id}`, String(next))
  }
  function toggleLimit() {
    const next = !limit; setLimit(next)
    localStorage.setItem(`fb-limit-${acc.id}`, String(next))
  }
  function toggleApple() {
    const next = !apple; setApple(next)
    localStorage.setItem(`fb-apple-${acc.id}`, String(next))
  }
  ...
  return (
    <ScreenOverlay title="Paramètres du portefeuille" onBack={onBack}>
      <div className="px-4 pt-5 pb-8 space-y-5">
        ...
        <div className="rounded-2xl overflow-hidden" style={{ background: '#fff' }}>
          {[
            { label: 'Paiements en ligne', desc: 'Autoriser les achats sur internet', on: online, toggle: toggleOnline },
            { label: 'Activer la limite',  desc: 'Définir un plafond mensuel',        on: limit,  toggle: toggleLimit  },
            { label: 'Ajouter à Apple Pay',desc: 'Payer avec votre iPhone / Apple Watch', on: apple, toggle: toggleApple },
          ].map(({ label, desc, on, toggle }, i) => (
...
// ── Block Confirm Modal ────────────────────────────────────────────────────────
function BlockConfirmModal({ acc, onCancel, onConfirm }: { acc: CurrencyAccount; onCancel: () => void; onConfirm: () => void }) {
  return (
...
function CardBackScreen({
  ...
  function toggleFrozen() {
    const next = !frozen; setFrozen(next)
    localStorage.setItem(`fb-frozen-${acc.id}`, String(next))
  }
  ...
  return (
    <ScreenOverlay title="Vue arrière" onBack={onBack}>
      {showBlock && (
        <BlockConfirmModal acc={acc} onCancel={() => setShowBlock(false)} onConfirm={() => { setShowBlock(false); onBlock() }} />
      )}
      <div className="px-4 pt-5 pb-8 space-y-5">
        {/* Card back */}
        <CardBack acc={acc} />
...
function FullSettingsScreen({ acc, onBack, onBlock }: {
  ...
  const items = [
    ...
    { icon: ArrowDownLeft, label: 'Paiements',           desc: 'Historique des transactions'       },
    { icon: RefreshCw,     label: 'Paramètre de carte',  desc: 'Limites et restrictions'           },
    { icon: Shield,        label: 'Remplacer',           desc: 'Demander un nouveau portefeuille'  },
  ]
...
function PinSetupScreen({
  ...
  return (
    <ScreenOverlay title={step === 'set' ? 'Créer votre code PIN' : 'Confirmer le PIN'} onBack={onBack}>
      <div className="flex flex-col items-center px-4 pt-8 pb-8 space-y-8">
        <div className="text-center">
          ...
          <p className="text-sm" style={{ color: 'rgba(13,27,75,0.55)' }}>
            {step === 'set' ? 'Entrez un code PIN à 4 chiffres pour ce portefeuille' : 'Saisissez à nouveau votre code PIN pour confirmer'}
          </p>
...
function AddWalletScreen({
  ...
  user: { user_metadata?: { full_name?: string }; email?: string } | null
  onBack: () => void
  onNext: (data: NewWalletData) => void
}) {
      ...
      <div className="px-4 pt-5 pb-8 space-y-5">
        ...
        <div className="relative rounded-[1.75rem] overflow-hidden w-full select-none"
          ...
          <div className="relative h-full flex flex-col justify-between p-5">
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-2.5">
                <div className="w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm" style={{ background: 'rgba(255,255,255,0.25)', color: '#fff' }}>
                  {getUserInitials(user)}
                </div>
                <div>
                  <p className="font-semibold text-white" style={{ fontSize: 11 }}>{holderName || 'Votre nom'}</p>
                  <p style={{ color: 'rgba(255,255,255,0.55)', fontSize: 9 }}>{getCurrency(currency)?.name}</p>
                </div>
              </div>
              <span className="font-bold px-2.5 py-0.5 rounded-full" style={{ background: 'rgba(255,255,255,0.2)', color: 'rgba(255,255,255,0.9)', fontSize: 10 }}>
                {currency}
            ...
            <div>
              ...
              <p className="font-bold text-white" style={{ fontSize: 26 }}>{getCurrency(currency)?.symbol} 0.00</p>
            </div>
            <div className="flex items-end justify-between">
        ...
        {step === 'form' ? (
          <>
            ...
            <div className="rounded-2xl overflow-hidden" style={{ background: '#fff' }}>
              ...
              <div className="px-4 py-3.5" style={{ borderBottom: '1px solid #F3F3F6' }}>
                <p className="text-xs font-medium mb-1" style={{ color: 'rgba(13,27,75,0.45)' }}>Numéro de portefeuille</p>
              ...
              <div className="px-4 py-3.5" style={{ borderBottom: '1px solid #F3F3F6' }}>
                ...
                <input
                  ...
                  className="w-full font-semibold text-sm bg-transparent outline-none"
                  style={{ color: '#0D1B4B' }}
                />
              </div>
              {/* Currency */}
              <div className="px-4 py-3.5" style={{ borderBottom: '1px solid #F3F3F6' }}>
                ...
                <select
                  ...
                  onChange={e => {
                    ...
                    setStyleId(CURRENCY_DEFAULT_STYLE[e.target.value] ?? 'purple')
                  }}
                  className="w-full font-semibold text-sm bg-transparent outline-none cursor-pointer"
                  style={{ color: '#0D1B4B' }}>
            ...
            {/* Card style picker */}
            <div>
              <p className="text-xs font-semibold mb-3" style={{ color: 'rgba(13,27,75,0.55)', letterSpacing: '0.08em' }}>DESIGN DE CARTE</p>
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={() => holderName.trim() && setStep('verify')}
          ...
          <>
            ...
            <div className="rounded-2xl overflow-hidden" style={{ background: '#fff' }}>
              ...
              <div className="px-4 py-4 flex items-center justify-between" style={{ borderBottom: '1px solid #F3F3F6' }}>
                <div>
                  ...
                  <p className="font-semibold text-sm" style={{ color: '#0D1B4B' }}>{getCurrency(currency)?.flag} {currency}</p>
                </div>
              </div>
            ...
            <button
              onClick={() => onNext({ holderName, currency, styleId, setDefault })}
...
function MainWalletScreen({
  ...
  const quickActions = [
    { label: 'Transférer', fn: () => navigate('/transfer') },
    { label: 'Recharger',  fn: () => {} },
    { label: 'Payer',      fn: () => navigate('/bills') },
  ]

  const settingsMenu = [
    { label: 'Portefeuille Info',    fn: () => activeAcc && onNavigate('details', activeAcc) },
    { label: 'Portefeuille Setting', fn: () => activeAcc && onNavigate('settings', activeAcc) },
    { label: 'Paiements',            fn: () => navigate('/history') },
  ]
                ...
                <div className="flex gap-1.5">
                  {sorted.map((_, i) => (
                    <button key={i} onClick={() => setCardIdx(i)}
                      className="cursor-pointer tr rounded-full"
              ...
              {sorted.map((acc, i) => {
                ...
                return (
                  <button key={acc.id} onClick={() => setCardIdx(i)}
                    ...
                    style={{
                      borderBottom: i < sorted.length - 1 ? '1px solid #F3F3F6' : 'none',
...
export function WalletPage() {
  ...
  function handleBlock() {
    if (activeAcc) {
      // Mark as blocked in localStorage for now
      localStorage.setItem(`fb-blocked-${activeAcc.id}`, 'true')
    }
    setShowBlock(false)
    setScreen('main')
  }
  ...
  async function handlePinConfirmed(pin: string) {
    ...
    const { data: newAcc, error } = await supabase
      ...
      .insert({
        ...
        balance: 0,
        is_main: pendingData.setDefault,
      })
      .select()
      .single()
    ...
    <div className="relative">
      ...
      {screen === 'add-wallet' && (
        <AddWalletScreen
          user={user}
          onBack={() => setScreen('main')}
          onNext={data => { setPendingData(data); setScreen('pin-setup') }}
        />
      )}
      {screen === 'pin-setup' && (
        <PinSetupScreen
          onBack={() => setScreen('add-wallet')}
          onConfirm={handlePinConfirmed}
        />
      )}

      {/* Main screen always in DOM */}
      <MainWalletScreen
        accounts={accounts}
        loading={loadingAcc}
        user={user}
